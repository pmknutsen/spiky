...
% PMK 08/10/11
% Drop spikes that dont conform to rule above
vIndxAll = 1:size(spikes.waveforms, 1);
vDropIndx = setdiff(vIndxAll, peak_inds(peaks_center,1));

%spikesbak = spikes;
%spikes = spikesbak

%tSpikesDrop.waveforms = spikes.waveforms(vDropIndx,:);
%spikes.waveforms(vDropIndx,:) = NaN;
%tSpikesDrop.spiketimes = spikes.waveforms(vDropIndx);
%spikes.spiketimes(vDropIndx) = NaN;

persistent pWarningIssued
if isempty(pWarningIssued), pWarningIssued = 0; end
if (length(peaks_center) ~= size(spikes.waveforms, 1)) && ~pWarningIssued
    sWarn = sprintf('Threshold informarion is invalid, as some waveforms do not have a central peak around the time of threshold crossing. These waveforms will be de-jittered by the median jitter of valid waveforms. This error will be suppressed in future calls to this function.');
    uiwait(warndlg(sWarn, 'Spiky!'));
    pWarningIssued = 1;
end
...
if (nargout > 2)
    ...
    %peak_locs(:, [1,3]) = peak_inds(peaks_center, [2,3]); % width info     PMK 08/11
    peak_locs(peak_inds(peaks_center,1), [1,3]) = peak_inds(peaks_center, [2,3]); % width info      PMK 08/11
